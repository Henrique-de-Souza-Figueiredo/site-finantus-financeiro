...
@app.route("/conversor_moedas", methods=["GET","POST"])
    ...
    if request.method == "POST":
        try:
            ...
            if moeda_origem == moeda_destino:
                resultado = f"{valor:.2f} {moeda_origem} = {valor:.2f} {moeda_destino}"
            else:
               
                if moeda_origem in taxas and moeda_destino in taxas[moeda_origem]:
                    taxa = taxas[moeda_origem][moeda_destino]
                    convertido = valor * taxa
                    resultado = f"{valor:.2f} {moeda_origem} = {convertido:.2f} {moeda_destino}"
                else:
                    resultado = "Combinação de moedas não suportada."
        except ValueError:
            resultado = "Por favor, insira um valor numérico válido."
        except Exception as e:
            resultado = f"Ocorreu um erro inesperado: {str(e)}"
 
    return render_template("conversor_moedas.html", resultado=resultado)
